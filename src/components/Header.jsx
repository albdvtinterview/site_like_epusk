...
const categoryNav = [
  ['Частотники', 'chastotnye-preobrazovateli', Activity],
  ['Устройства плавного пуска', 'ustroystva-plavnogo-puska', Settings2],
  ['Электродвигатели', 'elektrodvigateli', Cog],
  ['Промышленная автоматика', 'promyshlennaya-avtomatika', PanelsTopLeft],
  ['Мотор-редукторы', 'motor-reduktory', Grid3X3],
  ['Электропитание', 'elektropitanie', BatteryCharging],
  ['Тали', 'tali', Construction],
]
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white">
      ...
      <nav className="container-shell hidden h-14 items-center gap-7 overflow-hidden border-t border-slate-100 lg:flex" aria-label="Категории каталога">
        {categoryNav.map(([label, slug, Icon]) => (
          <a href={`#/catalog/${slug}`} key={label} className="flex shrink-0 items-center gap-2 whitespace-nowrap text-sm font-semibold text-ink transition hover:text-orange">
        ...
        <div className="absolute left-0 right-0 top-full border-t border-slate-100 bg-white px-5 pb-6 shadow-xl lg:hidden">
          <label className="relative my-4 block">
            ...
            <input
              ...
              onFocus={openCatalogSearch}
              className="h-12 w-full rounded-lg border border-slate-300 pl-4 pr-11 text-sm"
              placeholder="Поиск по каталогу"
            />
